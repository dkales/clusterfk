...
class MantisTrail(Trail.Trail):
    ...
    def getProbability(self,verbose=False):
        ...
        for col in range(4):
            ...
            total = len(self.states["S0"].at(0,col)) * len(self.states["S0"].at(1,col)) * len(self.states["S0"].at(2,col)) * len(self.states["S0"].at(3,col))
            for a,b,c,d in itertools.product(self.states["S0"].at(0, col), self.states["S0"].at(1, col), self.states["S0"].at(2, col), self.states["S0"].at(3, col)):
...
""")
        ...
        output.append(r"\caption{" + "Mantis-{R}: {S} active S-boxes, $2^{{{P:.2f}}}$ probability".format(R=numrounds, S=sum([1 if self.states["S"+str(s)].atI(i) != [0] else 0 for i in range(16) for s in range(self.rounds+1) + range(self.rounds+2,self.rounds*2+3)]), P=math.log(self.getProbability(),2)) + r"}")
        ...
        def state2sbs(xshift, yshift, sname, sName, vrcs):
            return r"  \drawstate[xshift=" + str(xshift) + " cm,yshift=" + str(yshift) + \
                        ...
                        COLORS.keys()[COLORS.values().index(self.colorlist[frozenset(vrcs[row,col])])] for row in rows for col in cols if vrcs[row, col] != [0]]) + \
                        r"}"
        ...
        def linkstate(Pin, Pout, fun="", extra=""):
            ...
            if fun: return r"  \draw[next] ("+Pin+r") -- node[above, font=\scriptsize] {"+fun+"} ("+Pout+r");"
            else:   return r"  \draw[next] ("+Pin+r") -- ("+Pout+r");"
        ...
        output.append(nextstate("xi_1", "ai_1") + " " + linkstate("ai_1_east", r"0,\hi-2") + " " + linkstate("th_1_north", "ai_1_south"))
        output.append(prevstate("xo_1", "ao_1") + " " + linkstate(r"0,\lo-2", "ao_1_east") + " " + linkstate("th_1_south", "ao_1_north"))
        ...
        for rnd in rnds:
            ...
            output.append(nextstate("Xi"+r, "ai"+r) + " " + nextstate("ai"+r, "yi"+r) + " " + linkstate("th"+r+"_north", "ai"+r+"_south"))
            output.append(prevstate("Xo"+r, "ao"+r) + " " + prevstate("ao"+r, "yo"+r) + " " + linkstate("th"+r+"_south", "ao"+r+"_north"))
            x += stepnext
        ...
        output.append(state2sbs(x, hi, "xi"+str(numrounds), "$x^"+tin +"_{"+str(numrounds)+"}$", getstate("S", "in",  numrounds)))
        output.append(state2sbs(x, lo, "xo"+str(numrounds), "$x^"+tout+"_{"+str(numrounds)+"}$", getstate("S", "out", numrounds)))
        x += stepnext
        output.append(state2sbs(x, hi, "Xi"+str(numrounds), "$x^"+tin +"_{"+str(numrounds)+"}$", getstate("A", "in",  numrounds+1)))
        output.append(state2sbs(x, lo, "Xo"+str(numrounds), "$x^"+tout+"_{"+str(numrounds)+"}$", getstate("a", "out", numrounds+1)))
        ...
        output.append(r"  \draw[next] (Xi"+str(numrounds)+r"_east) -| ++(1,\lo) node[right,font=\scriptsize] {\MC} |- (Xo"+str(numrounds)+"_east);")
        ...
        for i, (state, color) in enumerate(self.colorlist.items()):
            ...
            x = 5
            y = -15 - i * 2
            output.append(r"\node [draw, rectangle, fill=" + colorstring + r", minimum width=1, minimum height=1, label=right:{\tiny " + statestr + r"}] at (" + str(x) + "," + str(y) + r") {};") 
