    ...
    def redraw_state(self):
        ...
        for i in xrange(NUM_STATE_CELLS[0]):
            for j in xrange(NUM_STATE_CELLS[1]):
                ...
                self.canvas.create_rectangle(x, y, x+self.__dim["SIDE"], y+self.__dim["SIDE"], fill=color, tags="state")
                ...
                if self.state.statenumbers[i*NUM_STATE_CELLS[0] + j] != 0:
                    ...
                    self.canvas.create_text(x, y, text=str(self.state.statenumbers[i*NUM_STATE_CELLS[0] + j]), tags="state", fill="black")
...
class ClusterFK:
    ...
    def saveTrail(self):
        f = tkFileDialog.asksaveasfile(mode="w", defaultextension=".cfk")
